...
* AI Assistants
** gptel
=gptel= is an LLM chat client. On macOS it uses Claude Opus 4.5 via GitHub
Copilot (work setup). On WSL2 it switches to OpenAI/Codex (personal setup), with
Org mode as the default buffer format. =macher= provides agentic tool-use
capabilities.
#+begin_src elisp
(defun my/wsl-p ()
